    ...
    bindEventHandlers() {
        ...
        this.csgoClient.on('inspectItemInfo', (itemData) => {
            if (this.resolve && this.currentRequest) {
                ...
                // Process stickers for backward compatibility
                itemData.iteminfo.stickers = itemData.iteminfo.stickers.map(sticker => ({
                ...
                // Process keychains with all expected properties
                if (itemData.iteminfo.keychains && itemData.iteminfo.keychains.length > 0) {
                    itemData.iteminfo.keychains = itemData.iteminfo.keychains.map(keychains => ({
                        slot: keychains.slot,
                        keychainId: keychains.sticker_id,  // Use `sticker_id` here as it's used in your format
                        wear: keychains.wear,
                        scale: keychains.scale,
                        rotation: keychains.rotation,
                        tint_id: keychains.tint_id,
                        offset_x: keychains.offset_x,
                        offset_y: keychains.offset_y,
                        offset_z: keychains.offset_z,
                        pattern: keychains.pattern
                    }));
                } else {
                    itemData.iteminfo.keychains = [];
                }
