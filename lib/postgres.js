...
class Postgres {
    ...
    async ensureSchema() {
        ...
        await this.pool.query(`CREATE INDEX IF NOT EXISTS i_keychains ON items USING gin (keychains jsonb_path_ops) 
                            WHERE keychains IS NOT NULL`);
    ...
    async handleBulkInsert(data) {
        ...
        for (let [item, price] of data) {
            ...
            const keychains = item.keychains && item.keychains.length > 0 ? item.keychains.map((k) => ({
                slot: k.slot,
                pattern: k.pattern
            })) : null;
    ...
    async _getItemData(links) {
        ...
        return result.rows.map((item) => {
            ...
            item.keychains = item.keychains.map((k) => ({
                slot: k.slot,
                pattern: k.pattern
            }));
